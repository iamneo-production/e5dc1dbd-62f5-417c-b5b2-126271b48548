...
import com.hackathon.paymentservice.feign.InventoryService;
...
public class BillingServiceImpl implements BillingService{
    ...
    public BillingResponse billingOrder(BillingRequest billingRequest) {
        List<ProductItems> productsList=new ArrayList<>();
        ...
        for (Products productItem: orderDetails.getProductItemsList()){
            ProductItems products=new ProductItems(productItem.getProductId(),
        ...
        if (account == null || !passwordEncoder.matches(billingRequest.getSecretPin(), account.getSecretPin())) {
            ...
            PurchaseStatus updatedPurchaseStatus=new PurchaseStatus(purchaseStatus,orderDetails.getOrderId());

            orderUpdatedDetails=orderService.updatePurchaseStatus(updatedPurchaseStatus).getBody().getData();



            InventoryRequest inventoryRequest=new InventoryRequest(productsList);
            List<InventoryAllocation> inventoryAllocation=inventoryService.quantityCheck(inventoryRequest).getBody().getData();
        }
